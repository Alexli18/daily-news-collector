...
```bash
node helpers/sheets.js read-sources > /tmp/sources-sheet.json
```

Read `/tmp/sources-sheet.json`.
- If it contains a JSON array with at least one entry, use those as your source list (this is the `Sources` sheet tab overriding the repo file).
- If it is `null`, read `sources.json` from this repo instead.

Either way, keep only entries where `enabled` is `true`.
...
**`status` lifecycle (Story Clusters):** `pending` → `approved` → `script_generated`

- `pending` — default; not yet reviewed
- `approved` — manually set by an editor; ready for script generation ← **this step targets these**
- `script_generated` — set by `write-scripts.js` after this step; do not re-process

```bash
node helpers/read-clusters.js > /tmp/story-clusters.json
```

Read `/tmp/story-clusters.json`. Filter for rows where `status` is exactly `"approved"`. Ignore all other rows — `pending` and `script_generated` clusters must not be reprocessed.
...
- `draft` — generated by STEP 7; not yet reviewed
- `approved` — manually set by an editor; ready for production plan generation ← **this step targets these**
- `plan_generated` — set by `write-production-plans.js` after this step; do not re-process

```bash
node helpers/read-scripts.js > /tmp/scripts-tab.json
```

Read `/tmp/scripts-tab.json`. Filter for rows where `scriptStatus` is exactly `"approved"`. Ignore all other rows — `draft` and `plan_generated` scripts must not be reprocessed.
